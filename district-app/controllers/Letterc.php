...
class Letterc extends Admin_Controller
{
	...
	public function index($page = 1, $o = 0)
	{
		...
		$data['paging']  = $this->letterc_model->paging_letterc($page);
	...
	public function simpan_letterc($page = 1)
	{
		...
		$this->load->helper('form');
		$this->load->library('form_validation');
		$this->form_validation->set_rules('letterc', 'Nomor Surat C-DESA', 'required|trim|numeric');
		...
		if ($this->form_validation->run() != false) {
			...
			if ($this->input->post('id')) {
				...
				redirect("letterc/create_mutasi/{$id_letterc}");
			}
		} else {
			$_SESSION['success']   = -1;
