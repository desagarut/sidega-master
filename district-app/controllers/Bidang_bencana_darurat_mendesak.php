...
class Bidang_bencana_darurat_mendesak extends Admin_Controller {
	...
	{
		parent::__construct();
		$this->load->model(['bidang_bencana_darurat_mendesak_model', 'config_model', 'penduduk_model', 'keluarga_model']);
		$this->modul_ini = 370;
	...
	public function index($p = 1)
	{
		...
		$data = $this->bidang_bencana_darurat_mendesak_model->get_program($p, FALSE);
		...
		$this->render('bidang_bencana_darurat_mendesak/data_kejadian', $data);
	}

	public function form($program_id = 0)
	{
		$data['program'] = $this->bidang_bencana_darurat_mendesak_model->get_program(1, $program_id);
		$sasaran = $data['program'][0]['sasaran'];
		$nik = $this->input->post('nik');

		if (isset($nik))
		{
			$data['individu'] = $this->bidang_bencana_darurat_mendesak_model->get_peserta($nik, $sasaran);
			$data['individu']['program'] = $this->bidang_bencana_darurat_mendesak_model->get_peserta_program($sasaran, $data['individu']['id_peserta']);
		}
		else
		{
			$data['individu'] = NULL;
		...
		$data['form_action'] = site_url("bidang_bencana_darurat_mendesak/add_peserta/".$program_id);

		$this->render('bidang_bencana_darurat_mendesak/form', $data);
	}
	...
	public function detail($program_id = 0, $p = 1)
	{
		...
		$data['program'] = $this->bidang_bencana_darurat_mendesak_model->get_program($p, $program_id);
		$data['keyword'] = $this->bidang_bencana_darurat_mendesak_model->autocomplete($program_id, $this->input->post('cari'));
		...
		$this->render('bidang_bencana_darurat_mendesak/detail', $data);
	}

	// $id = program_peserta.id
	public function peserta($cat = 0, $id = 0)
	{
		$data = $this->bidang_bencana_darurat_mendesak_model->get_peserta_program($cat, $id);
	...
	{
		$data['peserta'] = $this->bidang_bencana_darurat_mendesak_model->get_bidang_bencana_darurat_mendesak_peserta_by_id($id);
		...
		$data['individu'] = $this->bidang_bencana_darurat_mendesak_model->get_peserta($peserta_id, $data['peserta']['sasaran']);
		$data['individu']['program'] = $this->bidang_bencana_darurat_mendesak_model->get_peserta_program($data['peserta']['sasaran'], $data['peserta']['peserta']);
		$data['detail'] = $this->bidang_bencana_darurat_mendesak_model->get_data_program($data['peserta']['program_id']);

		$this->set_minsidebar(1);

		$this->render('bidang_bencana_darurat_mendesak/profil_peserta', $data);
	}

	public function add_peserta($program_id = 0)
	{
		$this->bidang_bencana_darurat_mendesak_model->add_peserta($program_id);

		$redirect = ($this->session->userdata('aksi') != 1) ? $_SERVER['HTTP_REFERER'] : "bidang_bencana_darurat_mendesak/detail/$program_id";
	...
	public function aksi($aksi = '', $program_id = 0)
	{
		...
		redirect("bidang_bencana_darurat_mendesak/form/$program_id");
	}

	public function hapus_peserta($program_id = 0, $peserta_id = '')
	{
		$this->redirect_hak_akses('h', "bidang_bencana_darurat_mendesak/detail/$program_id");
		$this->bidang_bencana_darurat_mendesak_model->hapus_peserta($peserta_id);
		redirect("bidang_bencana_darurat_mendesak/detail/$program_id");
	}

	public function delete_all($program_id = 0)
	{
		$this->redirect_hak_akses('h', "bidang_bencana_darurat_mendesak/detail/$program_id");
		$this->bidang_bencana_darurat_mendesak_model->delete_all();
		redirect("bidang_bencana_darurat_mendesak/detail/$program_id");
	}
	...
	{
		$this->bidang_bencana_darurat_mendesak_model->edit_peserta($id);
	...
	{
		$data = $this->bidang_bencana_darurat_mendesak_model->get_bidang_bencana_darurat_mendesak_peserta_by_id($id);
	...
	public function form_laporan_kejadian()
	{
		$this->load->helper('form');
		$this->load->library('form_validation');

		$this->form_validation->set_rules('kelompok_bencana', 'Kelompok Bencana', 'required');
		$this->form_validation->set_rules('jenis_bencana', 'Jenis Bencana', 'required');
		...
		{
			$this->render('bidang_bencana_darurat_mendesak/data_kejadian_form', $data);
		}
		else
		{
			$this->bidang_bencana_darurat_mendesak_model->set_program();
			redirect("bidang_bencana_darurat_mendesak");
		}
	}

	// $id = program.id
	public function edit_laporan_kejadian($id = 0)
	{
		$this->load->helper('form');
		$this->load->library('form_validation');
		...
		$data['asaldana'] = unserialize(ASALDANA);
		$data['program'] = $this->bidang_bencana_darurat_mendesak_model->get_program(1, $id);
		$data['jml'] = $this->bidang_bencana_darurat_mendesak_model->jml_peserta_program($id);

		if ($this->form_validation->run() === FALSE)
		{
			$this->render('bidang_bencana_darurat_mendesak/data_kejadian_edit', $data);
		}
		else
		{
			$this->bidang_bencana_darurat_mendesak_model->update_program($id);
			redirect("bidang_bencana_darurat_mendesak");
		}
	}
	...
	{
		$this->bidang_bencana_darurat_mendesak_model->update_program($id);
		redirect("bidang_bencana_darurat_mendesak/detail/$id");
	}

	// $id = program.id
	public function hapus($id)
	{
		$this->redirect_hak_akses('h', "bidang_bencana_darurat_mendesak");
		$this->bidang_bencana_darurat_mendesak_model->hapus_program($id);
		redirect("bidang_bencana_darurat_mendesak");
	}
	...
	public function daftar($program_id = 0, $aksi = '')
	{
		if ($program_id > 0)
		{
			...
			$data['peserta'] = $this->bidang_bencana_darurat_mendesak_model->get_program(1, $program_id);
			$data['aksi'] = $aksi;
			$this->session->per_page = $temp;

			$this->load->view("bidang_bencana_darurat_mendesak/$aksi", $data);
		}
	}
	...
	public function impor()
	{
		...
		if ($this->upload->do_upload('userfile'))
		{
			...
			foreach ($reader->getSheetIterator() as $sheet)
			{
				...
				{
					$ambil_program = $this->bidang_bencana_darurat_mendesak_model->get_program(1, FALSE);
					...
					$program_id = $this->bidang_bencana_darurat_mendesak_model->impor_program($program_id, $data_program, $ganti_program);
				}

				// Sheet Peserta
				else
				{
					$ambil_peserta = $this->bidang_bencana_darurat_mendesak_model->get_program(1, $program_id);
					...
					foreach ($sheet->getRowIterator() as $row)
					{
						...
						$cek_peserta = $this->bidang_bencana_darurat_mendesak_model->cek_peserta($peserta, $sasaran);
					...
					{
						$this->bidang_bencana_darurat_mendesak_model->impor_peserta($program_id, $data_peserta, $kosongkan_peserta, $data_diubah);
					}
				}
			}
			$reader->close();
	...
	public function expor($program_id = '')
	{
		...
		$temp = $this->session->per_page;
		$this->session->per_page = 1000000000;
		$data = $this->bidang_bencana_darurat_mendesak_model->get_program(1, $program_id);
		$tbl_program = $data[0];
		$tbl_peserta = $data[1];

		//Nama File
		$writer = WriterEntityFactory::createXLSXWriter();
		$fileName = namafile('bidang_bencana_darurat_mendesak_' . $tbl_program['nama']) . '.xlsx';
