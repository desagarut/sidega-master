...
class Pemberdayaan_masyarakat_model extends MY_Model {
	...
	private function list_penduduk($id)
	{
		...
		$terdata = $this->get_id_peserta_penduduk($id);
	...
	private function list_kk($id)
	{
		...
		$terdata = $this->get_id_peserta_kk($id);
		...
		foreach ($data as $item)
		{
			...
			$kk = array(
				'id' => $item['id'],
				'nama_kegiatan' => strtoupper($item['nama_kegiatan']) ." [".$item['no_kk']."]",
				'info' => "RT/RW ". $item['rt']."/".$item['rw']." - ".strtoupper($item['dusun'])
			);
			$hasil[] = $kk;
		}
	...
	public function get_kegiatan($p, $kegiatan_id)
	{
		$kegiatan = $this->db->where('id', $kegiatan_id)->get('tbl_pemberdayaan_masyarakat')->row_array();

		switch ($kegiatan['sasaran'])
		{
			// Sasaran Penduduk
			case '1':
				$data = $this->get_penduduk_terdata($kegiatan_id, $p);
			...
			// Sasaran Keluarga
			case '2':
				$data = $this->get_kk_terdata($kegiatan_id, $p);
		...
		$data['keyword'] = $this->autocomplete($kegiatan['sasaran']);
	...
	private function get_penduduk_terdata_sql($kegiatan_id)
	{
		# Data penduduk
	...
	private function get_penduduk_terdata($kegiatan_id, $p)
	{
		$hasil = [];
		$get_terdata_sql = $this->get_penduduk_terdata_sql($kegiatan_id);
		...
		if ($query->num_rows() > 0)
		{
			...
			for ($i=0; $i<count($data); $i++)
			{
				...
				$data[$i]['terdata_nama'] = strtoupper($data[$i]['nama_kegiatan']);
	...
	private function get_kk_terdata_sql($kegiatan_id)
	{
		# Data KK
		$sql = " FROM tbl_pemberdayaan_masyarakat_peserta s
	...
	private function get_kk_terdata($kegiatan_id, $p)
	{
		$hasil = [];
		$get_terdata_sql = $this->get_kk_terdata_sql($kegiatan_id);
	...
	*/
	public function get_terdata($id_peserta, $sasaran)
	{
		...
		switch ($sasaran)
		{
			...
			case 1:
				...
				$data['terdata_nama'] = $data['nama_kegiatan'];
	...
	public function hapus_terdata($id_peserta)
	{
		$this->db->where('id', $id_peserta);
		$this->db->delete('tbl_pemberdayaan_masyarakat_peserta');
	}

	// $id = tbl_pemberdayaan_masyarakat_peserta.id
	...
	/*
		Mengambil data individu terdata menggunakan id tabel tbl_pemberdayaan_masyarakat_peserta
	*/
	public function get_peserta_terdata_by_id($id)
	{
		$data = $this->db->where('id', $id)->get('tbl_pemberdayaan_masyarakat_peserta')->row_array();
		// Data tambahan untuk ditampilkan
		$terdata = $this->get_terdata($data['id_peserta'], $data['sasaran']);
		switch ($data['sasaran'])
		{
			case 1:
				...
				$data['terdata_info'] = $terdata['nama_kegiatan'];
				break;
			case 2:
	...
	public function get_terdata_suplemen($sasaran, $id_peserta)
	{
		$list_suplemen = [];
		/*
		 * Menampilkan keterlibatan $id_peserta dalam data Kegiatan Pemberdayaan yang ada
		 *
		 * */
