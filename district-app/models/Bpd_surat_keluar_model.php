...
class Bpd_surat_keluar_model extends MY_Model
{
	...
	public function autocomplete()
	{
		...
		return $this->autocomplete_str('tujuan', 'bpd_tbl_surat_keluar');
	}
	...
	{
		$this->db->from('bpd_tbl_surat_keluar u');
		$this->search_sql();
		$this->filter_sql();
	}
	...
	{
		$query = $this->db->distinct()->select('YEAR(tanggal_surat) AS tahun')->order_by('YEAR(tanggal_surat)', 'DESC')->get('bpd_tbl_surat_keluar')->result_array();
		return $query;
	}

	/**
	 * Insert data baru ke tabel bpd_tbl_surat_keluar
	 * @return  void
	 */
	public function insert()
	{
		...
		// karakter maksimal yang bisa ditampung kolom bpd_tbl_surat_keluar.berkas_scan hanya 100 karakter
		...
		$indikatorSukses = is_null($uploadError) && $this->db->insert('bpd_tbl_surat_keluar', $data);
	...
	/**
	 * Update data di tabel bpd_tbl_surat_keluar
	...
	public function update($idSuratMasuk)
	{
		...
		if ($adaLampiran === TRUE) {
			...
			// karakter maksimal yang bisa ditampung kolom bpd_surat_keluar.berkas_scan hanya 100 karakter
			...
			if ($this->upload->do_upload('satuan')) {
				...
				$databaseUpdated = $this->db->update('bpd_tbl_surat_keluar', $data);
	...
	{
		$bpd_surat_keluar = $this->db->where('id', $id)->get('bpd_tbl_surat_keluar')->row_array();
		return $bpd_surat_keluar;
	}
	...
	public function delete($idSuratMasuk, $semua = false)
	{
		...
		if (!is_null($namaBerkas)) {
			...
			if (is_null($_SESSION['error_msg'])) {
				$hapusRecordDb = $this->db->where('id', $idSuratMasuk)->delete('bpd_tbl_surat_keluar');
	 ...
	 * @param  string       $idSuratMasuk  Id pada tabel bpd_tbl_surat_keluar
	...
	public function getNamaBerkasScan($idSuratMasuk)
	{
		...
		$sql = "SELECT berkas_scan FROM bpd_tbl_surat_keluar WHERE id = ? LIMIT 1;";
