...
class Bpd_surat_masuk_model extends MY_Model
{
	...
	public function autocomplete()
	{
		...
		return $this->autocomplete_str('pengirim', 'bpd_tbl_surat_masuk');
	}
	...
	{
		$sql = " FROM bpd_tbl_surat_masuk u WHERE 1 ";
		$sql .= $this->search_sql();
		$sql .= $this->filter_sql();
		return $sql;
	}
	...
	{
		$query = $this->db->distinct()->select('YEAR(tanggal_penerimaan) AS tahun')->order_by('YEAR(tanggal_penerimaan)', 'DESC')->get('bpd_tbl_surat_masuk')->result_array();
		return $query;
	}
	...
	/**
	 * Insert data baru ke tabel bpd_tbl_surat_masuk
	 * @return  void
	 */
	public function insert()
	{
		...
		// karakter maksimal yang bisa ditampung kolom bpd_tbl_surat_masuk.berkas_scan hanya 100 karakter
		...
		$indikatorSukses = is_null($uploadError) && $this->db->insert('bpd_tbl_surat_masuk', $data);
	...
	/**
	 * Update data di tabel bpd_tbl_surat_masuk
	...
	public function update($idSuratMasuk)
	{
		...
		if ($adaLampiran === TRUE) {
			...
			if ($this->upload->do_upload('satuan')) {
				...
				$databaseUpdated = $this->db->update('bpd_tbl_surat_masuk', $data);
	...
	{
		$surat_masuk = $this->db->where('id', $id)->get('bpd_tbl_surat_masuk')->row_array();
		return $surat_masuk;
	}
	...
	public function delete($idSuratMasuk, $semua = false)
	{
		...
		if (!is_null($namaBerkas)) {
			...
			if (is_null($_SESSION['error_msg'])) {
				$hapusRecordDb = $this->db->where('id', $idSuratMasuk)->delete('bpd_tbl_surat_masuk');
	 ...
	 * @param  string       $idSuratMasuk  Id pada tabel bpd_tbl_surat_masuk
	...
	public function getNamaBerkasScan($idSuratMasuk)
	{
		...
		$sql = "SELECT berkas_scan FROM bpd_tbl_surat_masuk WHERE id = ? LIMIT 1;";
