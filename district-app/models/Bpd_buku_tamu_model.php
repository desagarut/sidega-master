<?php class Bpd_buku_tamu_model extends MY_Model
{
	...
	public function __construct()
	{
		...
		$this->urut_model = new Urut_Model('bpd_tbl_buku_tamu');
	}

	public function autocomplete()
	{
		return $this->autocomplete_str('nama', 'bpd_tbl_buku_tamu');
	}
	...
	private function search_sql()
	{
		if (isset($_SESSION['cari'])) {
			...
			$kw = '%' . $kw . '%';
			$search_sql = " AND (gambar LIKE '$kw' OR nama LIKE '$kw' OR tahun LIKE '$kw')";
			return $search_sql;
		}
	}
	...
	{
		$sql = " FROM bpd_tbl_buku_tamu WHERE tipe = 0  ";
		$sql .= $this->search_sql();
		$sql .= $this->filter_sql();
		return $sql;
	}
	...
	public function insert()
	{
		...
		// Bolehkan buku tidak ada gambar cover
		...
		$outp = $this->db->insert('bpd_tbl_buku_tamu', $data);
	...
	public function update($id = 0)
	{
		...
		$outp = $this->db->where('id', $id)->update('bpd_tbl_buku_tamu', $data);
		if (!$outp) $_SESSION['success'] = -1;
	}
	...
	public function delete_gallery($id = '', $semua = false)
	{
		...
		$this->delete($id);
		$daftar_tamu = $this->db->select('id')->where('parrent', $id)->get('bpd_tbl_buku_tamu')->result_array();
		foreach ($daftar_tamu as $gallery) {
	...
	public function delete($id = '', $semua = false)
	{
		...
		$outp = $this->db->where('id', $id)->delete('bpd_tbl_buku_tamu');
	...
	{
		$image = $this->db->select('gambar')->get_where('bpd_tbl_buku_tamu', array('id' => $id))->row()->gambar;
	...
	public function get_buku($id = 0)
	{
		$sql = "SELECT * FROM bpd_tbl_buku_tamu WHERE id = ?";
	...
	{
		$sql = " FROM bpd_tbl_buku_tamu WHERE parrent = ? AND tipe = 2 ";
		$sql .= $this->search_sql();
		$sql .= $this->filter_sql();
		return $sql;
	}
	...
	public function insert_data_tamu($parrent = 0)
	{
		$_SESSION['success'] = 1;
		$_SESSION['error_msg'] = '';
		if (UploadError($_FILES['gambar'])) {
		...
		$data = [];
		$data['nama'] = $this->input->post('nama');
		$data['tanggal'] = tgl_indo_in($this->input->post('tanggal'));
		$data['jabatan'] = $this->input->post('jabatan');
		$data['alamat'] = $this->input->post('alamat');
		$data['keperluan'] = $this->input->post('keperluan');
		$data['urut'] = $this->urut_model->urut_max(array('parrent' => $parrent)) + 1;
		// Bolehkan isi buku tidak ada gambar
		if (!empty($lokasi_file)) {
			if (!CekGambar($_FILES['gambar'], $tipe_file)) {
				$_SESSION['success'] = -1;
				return;
	...
	public function update_data_tamu($id = 0)
	{
		$_SESSION['success'] = 1;
		$_SESSION['error_msg'] = '';
		if (UploadError($_FILES['gambar'])) {
