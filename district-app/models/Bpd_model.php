...
class Bpd_model extends CI_Model {
	...
	public function __construct()
	{
		...
		$this->urut_model = new Urut_Model('tweb_anggota_bpd', 'pamong_id');
	...
	public function autocomplete()
	{
		$sql = "SELECT * FROM
				...
				UNION SELECT pamong_nama FROM tweb_anggota_bpd
				UNION SELECT p.nik
					FROM tweb_anggota_bpd u
					LEFT JOIN tweb_penduduk p ON u.id_pend = p.id
				UNION SELECT pamong_nik FROM tweb_anggota_bpd
				UNION SELECT pamong_niap FROM tweb_anggota_bpd
				UNION SELECT pamong_nip FROM tweb_anggota_bpd";
	...
	{
		$pamong = $this->db->where('pamong_id', $id)->limit(1)->get('tweb_anggota_bpd')->row_array();;
	...
	public function insert()
	{
		...
		$outp = $this->db->insert('tweb_anggota_bpd', $data);
	...
	public function update($id=0)
	{
		...
		$data = $this->siapkan_data($data);
		$this->db->where("pamong_id", $id)->update('tweb_anggota_bpd', $data);
	}
	...
	public function delete($id='', $semua=false)
	{
		...
		$foto = $this->db->select('foto')->where('pamong_id',$id)->get('tweb_anggota_bpd')->row()->foto;
		if (!empty($foto))
		{
		...
		$outp = $this->db->where('pamong_id', $id)->delete('tweb_anggota_bpd');
	...
	public function ttd($jenis, $id, $val)
	{
		if ($val == 1)
		{
			...
			$this->db->where($jenis, 1)->update('tweb_anggota_bpd', [$jenis => 0]);
		}
		$this->db->where('pamong_id', $id)->update('tweb_anggota_bpd', [$jenis => $val]);
	}

	public function get_ttd()
	{
		$ttd = $this->db->where('pamong_ttd', 1)->get('tweb_anggota_bpd')->row_array();
		return $ttd;
	}

	public function get_ub()
	{
		$ub = $this->db->where('pamong_ub', 1)->get('tweb_anggota_bpd')->row_array();
		return $ub;
	}
	...
	public function list_penduduk()
	{
		$data = $this->db->select('u.id, u.nik, u.nama, w.dusun, w.rw, w.rt, u.sex')
			...
			->where('u.id NOT IN (SELECT id_pend FROM tweb_anggota_bpd WHERE id_pend IS NOT NULL)')
			->get()
			->result_array();
	...
	public function lock($id, $val)
	{
		$this->db
			->where('pamong_id', $id)
			->update('tweb_anggota_bpd', ['pamong_status' => $val]);
	}
    ...
    // ;
    	...
    	->where('pamong_status', 1)
			->get('tweb_anggota_bpd')->result_array();
		$data['struktur'] = [];
