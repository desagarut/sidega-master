...
class Penomoran_surat_bpd_model extends CI_Model
{
	...
	public function get_surat_terakhir($type, $url = null)
	{
		...
		if ($setting == 3) {
			$last_sl = $this->get_surat_terakhir_type('log_surat_bpd', null, 1);
	...
	private function get_surat_terakhir_type($type, $url = null, $setting = null)
	{
		...
		switch ($type) {
			default:
				show_error("Function $self(): Unknown type `$type`");
	...
	public function nomor_surat_duplikat($type, $nomor_surat, $url = null)
	{
		...
		if ($setting == 3) {
			...
			$sql = array();
			$sql[] = '(' . $this->db->from("log_surat_bpd")
				->select('no_surat as nomor_urut')
				->where('YEAR(tanggal)', $thn)
				->where('no_surat', $nomor_surat)
				->get_compiled_select()
				. ')';
			$sql[] = '(' . $this->db->from("bpd_tbl_surat_masuk")
				->select('nomor_urut')
				->where('YEAR(tanggal_surat)', $thn)
				->where('nomor_urut', $nomor_surat)
				->get_compiled_select()
				. ')';
			$sql[] = '(' . $this->db->from("bpd_tbl_surat_keluar")
				->select('nomor_urut')
				->where('YEAR(tanggal_surat)', $thn)
				->where('nomor_urut', $nomor_surat)
				->get_compiled_select()
