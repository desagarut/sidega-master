...
class Data_dtks_model extends MY_Model {
	...
	public function create()
	{
		...
		$hasil = $this->db->insert('data_dtks', $data);
	...
	public function list_data($sasaran = 0)
	{
		...
		$data = $this->db
			...
			->from('data_dtks s')
			->join('data_dtks_detail st', "s.id = st.id_data_dtks", 'left')
			->order_by('s.tahun')
			->group_by('s.id')
			->get()
			->result_array();
	...
	private function get_id_terdata_penduduk($id_data_dtks)
	{
		$list_penduduk = $this->db
			->select('p.id')
			->from('tweb_penduduk p')
			->join('data_dtks_detail t', 'p.id = t.id_terdata', 'left')
			->where('t.id_data_dtks', $id_data_dtks)
			->get()
			->result_array();
	...
	private function get_id_terdata_kk($id_data_dtks)
	{
		$list_kk = $this->db
			->select('k.id')
			->from('tweb_keluarga k')
			->join('data_dtks_detail t', 'k.id = t.id_terdata', 'left')
			->where('t.id_data_dtks', $id_data_dtks)
			->get()
			->result_array();
	...
	public function get_data_dtks($id)
	{
		$data = $this->db
			->select('s.*')
			->select('COUNT(st.id) AS jml')
	...
	public function get_rincian($p, $data_dtks_id)
	{
		$data_dtks = $this->db->where('id', $data_dtks_id)->get('data_dtks')->row_array();

		switch ($data_dtks['sasaran'])
		{
			// Sasaran Penduduk
			case '1':
				$data = $this->get_penduduk_terdata($data_dtks_id, $p);
			...
			// Sasaran Keluarga
			case '2':
				$data = $this->get_kk_terdata($data_dtks_id, $p);
				$data['judul']['judul_terdata_foto'] = 'FOTO';
		...
		$data['data_dtks'] = $data_dtks;
		$data['keyword'] = $this->autocomplete($data_dtks['sasaran']);
	...
	private function get_penduduk_terdata_sql($data_dtks_id)
	{
		# Data penduduk
		$sql = " FROM data_dtks_detail s
			...
			WHERE s.id_data_dtks=".$data_dtks_id;
		return $sql;
	}

	private function get_penduduk_terdata($data_dtks_id, $p)
	{
		$hasil = [];
		$get_terdata_sql = $this->get_penduduk_terdata_sql($data_dtks_id);
	...
	private function get_kk_terdata_sql($data_dtks_id)
	{
		# Data KK
		$sql = " FROM data_dtks_detail s
	...
	private function get_kk_terdata($data_dtks_id, $p)
	{
		$hasil = [];
		$get_terdata_sql = $this->get_kk_terdata_sql($data_dtks_id);
		$select_sql = "SELECT s.*, s.id_terdata, o.no_kk, s.id_data_dtks, o.nik_kepala, o.alamat, q.nik, q.nama, q.tempatlahir, q.tanggallahir, q.sex, q.foto, q.pekerjaan_id, w.rt, w.rw, w.dusun ";
		$sql = $select_sql.$get_terdata_sql;
	...
	*/
	...
	{
		$ada = $this->db->where('id_data_dtks', $id)
			->get('data_dtks_detail')->num_rows();
		if ($ada)
		{
		...
		$hasil = $this->db->where('id', $id)->delete('data_dtks');
	...
	{
		$data = $this->validasi($this->input->post());
		$hasil = $this->db->where('id', $id)->update('data_dtks', $data);
	...
	public function add_terdata($post, $id)
	{
		...
		$sasaran = $this->db->select('sasaran')->where('id', $id)->get('data_dtks')->row()->sasaran;
		$hasil = $this->db->where('id_data_dtks', $id)->where('id_terdata', $id_terdata)->get('data_dtks_detail');
		if ($hasil->num_rows() > 0)
		{
			return false;
		}
		else
		{
			...
			return $this->db->insert('data_dtks_detail', $data);
		}
				status_sukses($hasil); //Tampilkan Pesan
	...
	public function hapus_terdata($id_terdata)
	{
		...
		$this->db->delete('data_dtks_detail');
	}

	// $id = data_dtks_detail.id
	public function edit_terdata($post, $id)
	{
		$data['keterangan_padan'] = $post['keterangan_padan']; // Batasi 100 karakter
		$data['keterangan_bantuan'] = substr(htmlentities($post['keterangan_bantuan']), 0, 100); // Batasi 100 karakter

		$data['id_dtks'] = $post['id_dtks'];
		$this->db->where('id', $id);
		$this->db->update('data_dtks_detail', $data);
	}

	/*
		Mengambil data individu terdata menggunakan id tabel data_dtks_detail
	*/
	public function get_data_dtks_terdata_by_id($id)
	{
		$data = $this->db->where('id', $id)->get('data_dtks_detail')->row_array();
	...
	public function get_terdata_data_dtks($sasaran,$id_terdata)
	{
		$list_data_dtks = [];
		/*
		 * Menampilkan keterlibatan $id_terdata dalam data data_dtks yang ada
		 *
		 * */
		$strSQL = "SELECT p.id as id, o.id_terdata as nik, p.nama as nama
			FROM data_dtks_detail o
			LEFT JOIN data_dtks p ON p.id = o.id_data_dtks
			WHERE ((o.id_terdata='".$id_terdata."') AND (o.sasaran='".$sasaran."'))";
		$query = $this->db->query($strSQL);
		if ($query->num_rows() > 0)
		{
			$list_data_dtks = $query->result_array();
		}
		...
		if ( ! empty($list_data_dtks))
		{
			$hasil = array("daftar_data_dtks" => $list_data_dtks, "profil" => $data_profil);
			return $hasil;
		}
		else
		{
