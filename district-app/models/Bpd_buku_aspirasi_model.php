...
class Bpd_buku_aspirasi_model extends MY_Model
{

	// Untuk datatables peserta bantuan di themes/hijau/partials/statistik.php (web)
	var $column_order = array(null, 'bpd_tbl_aspirasi', 'peserta', null); //set column field database for datatable orderable
	...
	{
		if ($sasaran > 0) {
			$strSQL = "SELECT p.id, p.nama, p.tahun, p.sasaran, p.ndesc, p.sdate, p.edate, p.userid, p.status
				FROM bpd_tbl_aspirasi p WHERE p.sasaran=" . $sasaran;
		} else {
			$strSQL = "SELECT p.id, p.nama, p.tahun, p.sasaran, p.ndesc, p.sdate, p.edate, p.userid, p.status, CONCAT('50',p.id) as lap
				FROM bpd_tbl_aspirasi p WHERE 1";
		}
	...
	public function list_program_keluarga($kk_id)
	{
		...
		$sasaran = 2;
		$strSQL = "
			SELECT p.id, p.nama, p.tahun, p.sasaran, p.ndesc, p.sdate, p.edate, p.userid, p.status, CONCAT('50',p.id) as lap, pp.peserta
			FROM bpd_tbl_aspirasi p
			LEFT OUTER JOIN bpd_tbl_aspirasi_detail pp ON p.id = pp.program_id AND pp.peserta = '$no_kk'
	...
	*/
	...
	private function get_peserta_sql($slug, $sasaran, $jumlah = false)
	{
		...
		switch ($sasaran) {
			case 1:
				...
				$strSQL = "SELECT " . $select_sql . " FROM bpd_tbl_aspirasi_detail p
			...
			case 2:
				...
				$strSQL = "SELECT " . $select_sql . "
					FROM bpd_tbl_aspirasi_detail p
	...
	public function get_sasaran($id)
	{
		...
		$this->db->where('id', $id);
		$query = $this->db->get('bpd_tbl_aspirasi');
	...
	{
		$sql = ' FROM bpd_tbl_aspirasi p WHERE 1 ';
	...
	private function get_program_data($p, $slug)
	{
		...
		switch ($hasil0["sasaran"]) {
			case 1:
				 ...
				 * Data penduduk
				 * */
				$hasil0['judul_peserta'] = 'Nama Penduduk Pemberi Aspirasi';
			...
			case 3:
				 ...
				 * Data RTM
				 * */
				$hasil0['judul_peserta'] = 'Nama Rumah Tangga Pemberi Aspirasi';
			...
			case 4:
				 ...
				 * Data Kelompok
				 * */
				$hasil0['judul_peserta'] = 'Nama Kelompok Pemberi Aspirasi';
	...
	private function get_pilihan_penduduk($filter)
	{
		...
		if ($query->num_rows() > 0) {
			$j = 0;
			for ($i = 0; $i < count($data); $i++) {
				// Abaikan penduduk yang sudah terdaftar pada bpd_tbl_aspirasi
	...
	private function get_pilihan_kk($filter)
	{
		...
		if ($query->num_rows() > 0) {
			$j = 0;
			for ($i = 0; $i < count($data); $i++) {
				// Abaikan keluarga yang sudah terdaftar pada bpd_tbl_aspirasi
	...
	private function get_pilihan_rumah_tangga($filter)
	{
		...
		if ($query->num_rows() > 0) {
			$j = 0;
			for ($i = 0; $i < count($data); $i++) {
				// Abaikan RTM yang sudah terdaftar pada bpd_tbl_aspirasi
	...
	private function get_pilihan_kelompok($filter)
	{
		...
		if ($query->num_rows() > 0) {
			$j = 0;
			for ($i = 0; $i < count($data); $i++) {
				// Abaikan kelompok yang sudah terdaftar pada bpd_tbl_aspirasi
				if (!in_array($data[$i]['id'], $filter)) {
					$hasil2[$j]['id'] = $data[$i]['id'];
	...
	public function get_buku_aspirasi($p, $slug)
	{
		if ($slug === false) {
			//Query untuk expiration status, jika end date sudah melebihi dari datenow maka status otomatis menjadi tidak aktif
			$expirySQL = "UPDATE bpd_tbl_aspirasi SET status = IF(edate < CURRENT_DATE(), 0, IF(edate > CURRENT_DATE(), 1, status)) WHERE status IS NOT NULL";
			$expiryQuery = $this->db->query($expirySQL);

			$response['paging'] = $this->paging_bantuan($p);
			$strSQL = "SELECT COUNT(v.program_id) AS jml_peserta, p.id, p.nama, p.tahun, p.sasaran, p.ndesc, p.sdate, p.edate, p.userid, p.status FROM bpd_tbl_aspirasi p ";
			$strSQL .= "LEFT JOIN bpd_tbl_aspirasi_detail  AS v ON p.id = v.program_id WHERE 1 ";
			$strSQL .= $this->sasaran_sql();
			$strSQL .= " GROUP BY p.id ";
			$strSQL .= ' LIMIT ' . $response["paging"]->offset . ',' . $response["paging"]->per_page;
			$query = $this->db->query($strSQL);
			$data = $query->result_array();
			$response['buku_aspirasi'] = $data;
			return $response;
		} else {
	...
	// Ambil data bpd_tbl_aspirasi
	function get_data_program($id)
	{
		// Untuk bpd_tbl_aspirasi, $id '50<program_id>'
		$program_id = preg_replace("/^50/", "", $id);
		return $this->db->select("*")->where("id", $program_id)->get("bpd_tbl_aspirasi")->row_array();
	}

	/*
	 * Fungsi untuk menampilkan bpd_tbl_aspirasi bantuan yang sedang diterima peserta.
	 * $id => id_peserta tergantung sasaran
	 * $cat => sasaran bpd_tbl_aspirasi bantuan.
	 *
	 * */
	public function get_peserta_program($cat, $id)
	{
		...
		$strSQL = "SELECT p.id AS id, o.peserta AS nik, o.id AS peserta_id,  p.nama AS nama, p.sdate, p.edate, p.ndesc, p.status
			FROM bpd_tbl_aspirasi_detail o
			LEFT JOIN bpd_tbl_aspirasi p ON p.id = o.program_id
	...
	public function set_program()
	{
		...
		return $this->db->insert('bpd_tbl_aspirasi', $data);
	}
	...
	public function aspirasi_detail_add($program_id)
	{
		...
		$data['aspirasiyangdisampaikan'] = $this->input->post('aspirasiyangdisampaikan');
		$data['sasaran'] = $this->input->post('sasaran');

		$file_gambar = $this->_upload_gambar();
		if ($file_gambar) $data['dokumen'] = $file_gambar;
		$outp = $this->db->insert('bpd_tbl_aspirasi_detail', $data);
		status_sukses($outp, true);
	}

	// $id = bpd_tbl_aspirasi_detail.id
	public function aspirasi_detail_edit($id)
	{
		$this->session->success = 1;
		$this->session->error_msg = '';
		$data = $this->validasi_peserta($this->input->post());
		...
		$this->db->where('id', $id);
		$outp = $this->db->update('bpd_tbl_aspirasi_detail', $data);
		status_sukses($outp, true);
	}
	...
	//	$data['tanggal'] 	= date_is_empty($post['tanggal']) ? NULL : tgl_indo_in($post['tanggal']);
	...
	public function hapus_peserta_program($peserta_id, $program_id)
	{
		...
		$this->db->delete('bpd_tbl_aspirasi_detail');
	}
	...
	/*
		Mengambil data individu peserta menggunakan id tabel bpd_tbl_aspirasi_detail
	*/
	public function get_buku_pemberi_aspirasi_detail_by_id($id)
	{
		$data = $this->db
			->select('pp.*, p.sasaran')
			->from('bpd_tbl_aspirasi_detail pp')
			->join('bpd_tbl_aspirasi p', 'pp.program_id = p.id')
			->where('pp.id', $id)
			->get()
			->row_array();
		...
		switch ($data['sasaran']) {
			case 1:
				...
				$data['judul_peserta_info'] = 'Nama Penduduk';
	...
	public function update_buku_aspirasi($id)
	{
		$data = $this->validasi_bantuan($this->input->post());
		$hasil = $this->db->where('id', $id)
			->update('bpd_tbl_aspirasi', $data);

		if ($hasil) {
			$_SESSION["success"] = 1;
			$_SESSION["pesan"] = "Data Tabel Aspirasi telah diperbarui";
		} else {
			$_SESSION["success"] = -1;
		}
	}
	...
	{
		$jml_peserta = $this->db->select('count(v.program_id) as jml')->from('bpd_tbl_aspirasi p')->join('bpd_tbl_aspirasi_detail v', 'p.id = v.program_id', 'left')->where('p.id', $id)->get()->row()->jml;

		return $jml_peserta;
	}

	/*
		bpd_tbl_aspirasi yang sudah ada pesertanya tidak boleh dihapus
	*/
	public function hapus_program($id)
	{
		...
		$hasil = $this->db->where('id', $id)->delete('bpd_tbl_aspirasi');
		if ($hasil) {
			$_SESSION["success"] = 1;
			$_SESSION["pesan"] = "Data Buku Aspirasi telah dihapus";
		} else {
			$_SESSION["success"] = -1;
		}
	}
	...
	/* Mendapatkan daftar bantuan yang diterima oleh penduduk
		 parameter pencarian yang digunakan adalah nik ( data nik disimpan pada kolom peserta tabel bpd_tbl_aspirasi_detail ).
		 Saat ini terbatas pada bpd_tbl_aspirasi bantuan perorangan
	*/
	public function daftar_bantuan_yang_diterima($nik)
	{
		return $this->db->select('p.*, pp.*')
			->where(array('peserta' => $nik))
			->join('bpd_tbl_aspirasi p', 'p.id = pp.program_id', 'left')
			->get('bpd_tbl_aspirasi_detail pp')
			->result_array();
	}
	...
	{
		$this->db
			->select("p.nama as bpd_tbl_aspirasi, pend.nama as peserta, concat('RT ', w.rt, ' / RW ', w.rw, ' DUSUN ', w.dusun) AS alamat")
			->from('bpd_tbl_aspirasi p')
			->join('bpd_tbl_aspirasi_detail pp', 'p.id = pp.program_id', 'left');
	...
	public function impor_program($program_id = NULL, $data_program = [], $ganti_program = 0)
	{
		...
		if ($program_id == NULL) {
			$this->db->insert('bpd_tbl_aspirasi', $data_program);

			return $this->db->insert_id();
		}

		if ($ganti_program == 1) $this->db->where('id', $program_id)->update('bpd_tbl_aspirasi', $data_program);
	...
	public function impor_peserta($program_id = '', $data_peserta = [], $kosongkan_peserta = 0, $data_diubah = '')
	{
		...
		if ($kosongkan_peserta == 1) $this->db->where('program_id', $program_id)->delete('bpd_tbl_aspirasi_detail');

		if ($data_diubah) {
			$data_diubah = explode(", ", ltrim($data_diubah, ", "));

			$this->db->where_in('peserta', $data_diubah)->where('program_id', $program_id)->delete('bpd_tbl_aspirasi_detail');
		}

		$outp = $this->db->insert_batch('bpd_tbl_aspirasi_detail', $data_peserta);
		status_sukses($outp, true);
	}
