...
class Pembinaan_masyarakat_model extends MY_Model
{
	...
	public function autocomplete($id, $cari)
	{
		...
		$tabel = "tbl_pembinaan_masyarakat_peserta";
		$where = "program_id = $id";
	...
	{
		$this->load->model('keluarga_model'); // Di-load di sini karena tidak bisa diload di constructor, karena keluarga_model juga load pembinaan_masyarakat_model
		...
		$strSQL = "
			...
			FROM tbl_pembinaan_masyarakat p
			LEFT OUTER JOIN tbl_pembinaan_masyarakat_peserta pp ON p.id = pp.program_id AND pp.peserta = '$no_kk'
	...
	*/
	...
	private function get_peserta_sql($slug, $sasaran, $jumlah = false)
	{
		...
		switch ($sasaran) {
			case 1:
				...
				$strSQL = "SELECT " . $select_sql . " FROM tbl_pembinaan_masyarakat_peserta p
			...
			case 2:
				...
				$strSQL = "SELECT " . $select_sql . "
					FROM tbl_pembinaan_masyarakat_peserta p
	...
	public function get_sasaran($id)
	{
		...
		$this->db->where('id', $id);
		$query = $this->db->get('tbl_pembinaan_masyarakat');
	...
	{
		$sql = ' FROM tbl_pembinaan_masyarakat p WHERE 1 ';
	...
	public function get_program($p, $slug)
	{
		if ($slug === false) {
			...
			$expirySQL = "UPDATE tbl_pembinaan_masyarakat SET status = IF(edate < CURRENT_DATE(), 0, IF(edate > CURRENT_DATE(), 1, status)) WHERE status IS NOT NULL";
			$expiryQuery = $this->db->query($expirySQL);

			$response['paging'] = $this->paging_bantuan($p);
			$strSQL = "SELECT COUNT(v.program_id) AS jml_peserta, p.id, p.nama, p.penyelenggara, p.anggaran, p.sasaran, p.ndesc, p.sdate, p.edate, p.userid, p.status, p.asaldana FROM tbl_pembinaan_masyarakat p ";
			$strSQL .= "LEFT JOIN tbl_pembinaan_masyarakat_peserta AS v ON p.id = v.program_id WHERE 1 ";
	...
	public function get_peserta_program($cat, $id)
	{
		...
		$strSQL = "SELECT p.id AS id, o.peserta AS nik, o.id AS peserta_id,  p.nama AS nama, p.sdate, p.edate, p.ndesc, p.status
			FROM tbl_pembinaan_masyarakat_peserta o
			LEFT JOIN tbl_pembinaan_masyarakat p ON p.id = o.program_id
	...
	public function set_program()
	{
		...
		return $this->db->insert('tbl_pembinaan_masyarakat', $data);
	}
	...
	public function add_peserta($program_id)
	{
		...
		$outp = $this->db->insert('tbl_pembinaan_masyarakat_peserta', $data);
		status_sukses($outp, true);
	}

	// $id = tbl_pembinaan_masyarakat_peserta.id
	public function edit_peserta($id)
	{
		...
		$this->db->where('id', $id);
		$outp = $this->db->update('tbl_pembinaan_masyarakat_peserta', $data);
		status_sukses($outp, true);
	}
	...
	public function hapus_peserta_program($peserta_id, $program_id)
	{
		...
		$this->db->delete('tbl_pembinaan_masyarakat_peserta');
	}
	...
	/*
		Mengambil data individu peserta menggunakan id tabel tbl_pembinaan_masyarakat_peserta
	*/
	public function get_pembinaan_masyarakat_peserta_by_id($id)
	{
		$data = $this->db
			->select('pp.*, p.sasaran')
			->from('tbl_pembinaan_masyarakat_peserta pp')
			->join('tbl_pembinaan_masyarakat p', 'pp.program_id = p.id')
			->where('pp.id', $id)
			->get()
			->row_array();
	...
	public function update_program($id)
	{
		...
		$hasil = $this->db->where('id', $id)
			->update('tbl_pembinaan_masyarakat', $data);
	...
	{
		$jml_peserta = $this->db->select('count(v.program_id) as jml')->from('program p')->join('tbl_pembinaan_masyarakat_peserta v', 'p.id = v.program_id', 'left')->where('p.id', $id)->get()->row()->jml;
	...
	*/
	public function hapus_program($id)
	{
		...
		$hasil = $this->db->where('id', $id)->delete('tbl_pembinaan_masyarakat');
		if ($hasil) {
			$_SESSION["success"] = 1;
	...
	/* Mendapatkan daftar bantuan yang diterima oleh penduduk
		 parameter pencarian yang digunakan adalah nik ( data nik disimpan pada kolom peserta tabel tbl_pembinaan_masyarakat_peserta ).
	...
	*/
	public function daftar_bantuan_yang_diterima($nik)
	{
		return $this->db->select('p.*, pp.*')
			...
			->get('tbl_pembinaan_masyarakat_peserta pp')
			->result_array();
	}
	...
	private function get_all_peserta_bantuan_query()
	{
		$this->db
			...
			->from('program p')
			->join('tbl_pembinaan_masyarakat_peserta pp', 'p.id = pp.program_id', 'left');
	...
	public function impor_program($program_id = NULL, $data_program = [], $ganti_program = 0)
	{
		...
		if ($program_id == NULL) {
			$this->db->insert('tbl_pembinaan_masyarakat', $data_program);

			return $this->db->insert_id();
		}

		if ($ganti_program == 1) $this->db->where('id', $program_id)->update('tbl_pembinaan_masyarakat', $data_program);
	...
	public function impor_peserta($program_id = '', $data_peserta = [], $kosongkan_peserta = 0, $data_diubah = '')
	{
		...
		if ($kosongkan_peserta == 1) $this->db->where('program_id', $program_id)->delete('tbL_pembinaan_masyarakat');

		if ($data_diubah) {
			$data_diubah = explode(", ", ltrim($data_diubah, ", "));

			$this->db->where_in('peserta', $data_diubah)->where('program_id', $program_id)->delete('tbL_pembinaan_masyarakat');
		}

		$outp = $this->db->insert_batch('tbL_pembinaan_masyarakat', $data_peserta);
		status_sukses($outp, true);
	}
