			...
			<form id="validasi" action="<?= $form_action; ?>" method="POST" class="form-horizontal">
				<div class="box-body">
					...
					<div class="form-group">
						...
						<div class="col-sm-6">
							<select class="form-control select2 required" id="kursus" name="kursus">
								<option value="" selected="selected">-- Pilih Kursus --</option>
								<?php foreach ($daftar_kursus as $kursus) : ?>
									<option value="<?= $kursus['nama']; ?>" <?= selected($main['id'], $kursus['id']); ?>> <?= $kursus['nama']; ?></option>
								<?php endforeach; ?>
							</select>
					...
					<div class="form-group">
						...
						<div class="col-sm-6">
							<select class="form-control select2 required" id="bidang" name="bidang">
								<option value="" selected="selected">-- Pilih Bidang Keahlian --</option>
								<?php foreach ($daftar_bidang as $bidang) : ?>
									<option value="<?= $bidang['nama']; ?>" <?= selected($main['id'], $bidang['id']); ?>> <?= $bidang['nama']; ?></option>
								<?php endforeach; ?>
							</select>
