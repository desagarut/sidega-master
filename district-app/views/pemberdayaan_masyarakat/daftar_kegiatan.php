									...
									<?php if ($pemas): ?>
										<?php	foreach ($pemas as $key => $item):	?>
											<tr>
												...
												<td class="aksi">
													<a href="<?= site_url("pemberdayaan_masyarakat/clear/$item[id]"); ?>" class="btn bg-purple btn-box btn-sm" title="daftar_peserta Data"><i class="fa fa-list-ol"></i></a>
													...
													<a
														<?php if ($item['jml'] <= 0): ?>
															href="#" data-href="<?= site_url("pemberdayaan_masyarakat/hapus/$item[id]")?>" data-toggle="modal" data-target="#confirm-delete"
														<?php endif; ?>
												...
												<td class="padat"><?= $item['nama_penyelenggara']?></td>												
												<td class="padat"><?= tgl_indo_out($item['tgl_mulai'])?> - <?= tgl_indo_out($item['tgl_selesai'])?></td>
