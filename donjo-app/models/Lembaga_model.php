...
class Lembaga_model extends MY_Model {
	...
	{
		return $this->autocomplete_str('nama', 'lembaga');
	}
	...
	{
		$sql = "FROM lembaga u
			LEFT JOIN lembaga_master s ON u.id_master = s.id
	...
	public function list_data($o = 0, $offset = 0, $limit = 0)
	{
		...
		$select_sql = "SELECT u.*, s.lembaga AS master, c.nama AS ketua, (SELECT COUNT(id) FROM lembaga_anggota WHERE id_lembaga = u.id) AS jml_anggota ";
	...
	public function insert()
	{
		...
		$outpa = $this->db->insert('lembaga', $data);
		$insert_id = $this->db->insert_id();

		$outpb = $this->db
			->set('id_lembaga', $insert_id)
			->set('id_penduduk', $data['id_ketua'])
			->set('no_anggota', 1)
			->set('jabatan', 1)
			->set('keterangan', 'Ketua lembaga') // keterangan default untuk Ketua lembaga
			->insert('lembaga_anggota');
	...
	public function insert_a($id = 0)
	{
		...
		$data['id_lembaga'] = $id;
		$data['foto'] = $nama_file;
		$this->ubah_jabatan($data['id_lembaga'], $data['id_penduduk'], $data['jabatan'], NULL);

		$sdh_ada = $this->db
			->select('id')
			->from('lembaga_anggota')
			->where('id_lembaga', $id)
			->where('id_penduduk', $data['id_penduduk'])
			->get()->row_array();
		if ( ! $sdh_ada)
		{
			$outp = $this->db->insert('lembaga_anggota', $data);
		}
	...
	public function update($id = 0)
	{
		...
		$this->db->where('id', $id);
		$outp = $this->db->update('lembaga', $data);
	...
	public function delete($id = '', $semua = FALSE)
	{
		...
		$outp = $this->db->where('id', $id)->delete('lembaga');
	...
	public function delete_anggota($id = '', $semua = FALSE)
	{
		...
		$outp = $this->db->where('id', $id)->delete('lembaga_anggota');
	...
	public function get_lembaga($id = 0)
	{
		$data = $this->db
			->select('k.*, km.lembaga AS kategori, tp.nama AS nama_ketua')
			->from('lembaga k')
			->join('lembaga_master km', 'k.id_master = km.id', 'left')
	...
	public function get_ketua_lembaga($id)
	{
		$this->load->model('penduduk_model');
		$sql = "SELECT u.id, u.nik, u.nama, k.id as id_lembaga, k.nama as nama_lembaga, u.tempatlahir, u.tanggallahir, s.nama as sex,
	...
	{
		$sql = "SELECT * FROM lembaga_anggota WHERE id_lembaga = ? AND id_penduduk = ?";
	...
	{
		$sql = "SELECT * FROM lembaga_master";
		$query = $this->db->query($sql);
		return $query->result_array();
	}

	private function in_list_anggota($lembaga)
	{
		$anggota = $this->db
			->select('p.id')
			->from('lembaga_anggota k')
			->join('penduduk_hidup p', 'k.id_penduduk = p.id', 'left')
			->where('k.id_lembaga', $lembaga)
			->get()->result_array();
		return sql_in_list(array_column($anggota, 'id'));
	}

	public function list_penduduk($ex_lembaga = '')
	{
		if ($ex_lembaga)
		{
			$anggota = $this->in_list_anggota($ex_lembaga);
	...
	public function list_pengurus($id_lembaga)
	{
		$this->db->where('jabatan <>', 90);
		$data = $this->list_anggota($id_lembaga);
		return $data;
	}

	public function list_anggota($id_lembaga = 0, $sub = '')
	{
		...
		$data = $this->db
			...
			->where('ka.id_lembaga', $id_lembaga)
	...
	public function ubah_jabatan($id_lembaga, $id_penduduk, $jabatan, $jabatan_lama)
	{
		// jika ada orang lain yang sudah jabat KETUA ubah jabatan menjadi anggota
		// update id_ketua lembaga di tabel lembaga
		if ($jabatan == '1') // Ketua
		{
			$this->db
		...
		{
			// jika yang diubah adalah jabatan KETUA maka kosongkan id_ketua lembaga di tabel lembaga
			$this->db
				->set('id_ketua', -9999) // kolom id_ketua di tabel lembaga tidak bisa NULL
				->where('id', $id_lembaga)
				->update('lembaga');
		}
	}
