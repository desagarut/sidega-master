...
<script>
	$(document).ready(function() {
		$('#example1').DataTable({
			...
			'ajax': {
				'url': "<?= site_url('perencanaan_desa/hasil_polling') ?>",
				'method': 'POST',
				'data': function(d) {
					d.tahun = $('#tahun').val();
				}
				...
				},
				{
					'data': function(data) {
						...
						if (data.status_vote == 1) {
							status_vote = `<a href="<?= site_url('perencanaan_desa_polling/tanggapan_per_item/') ?>${data.id}" id="status_vote" class="btn btn-icon btn-primary btn-sm mb-2 mr-2" title="Penentuan Prioritas Aktif">Lihat Detail</a>`
						} else {
