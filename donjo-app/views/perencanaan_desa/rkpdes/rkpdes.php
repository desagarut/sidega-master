							...
							<div class="col-md-12">
								<div class="box-header">
									<div class="col-md-12">
										<div class="row">
											<div class="col-md-1">
											<a href="<?= site_url("perencanaan_desa/dialog_cetak_rkpdes/{$perencanaan_desa->id}/cetak") ?>" class="btn bg-purple btn-sm" data-remote="false" data-toggle="modal" data-target="#modalBox" data-title="Cetak Data RKPDes" title="Cetak Data RKPDes"><i class="fa fa-print "></i> Cetak</a>&nbsp;
											</div>
...
<script>
	$(document).ready(function() {
		$('#example1').DataTable({
			...
			'ajax': {
				'url': "<?= site_url('perencanaan_desa/rkpdes') ?>",
				'method': 'POST',
				'data': function(d) {
					d.tahun = $('#tahun').val();
				}
				...
				},
				{
					'data': function(data) {
						...
						if (data.status_rkpdes == 1) {
							status_rkpdes = `<a href="<?= site_url('perencanaan_desa/apbdes_aktiv/') ?>${data.id}" class="btn btn-success btn-sm mb-2 mr-2 disabled" title="APBDes Aktiv">Ditetapkan <br/>Dalam APBDes</a>`
						} else if (data.status_rkpdes == 0) {
							status_rkpdes = `<a href="<?= site_url('perencanaan_desa/durkp_aktiv/') ?>${data.id}" class="btn  btn-secondary btn-sm mb-2 mr-2 disabled" title="DURKP Aktiv">Ditetapkan <br/> Dalam DURKP</a>`
						} else {
							status_rkpdes = `<a href="<?= site_url('perencanaan_desa/durkp_aktiv/') ?>${data.id}" class="btn btn-warning btn-sm mb-2 mr-2 disabled" title="DURKP Aktiv">Silahkan Tetapkan<br/> APBdes / DURKP</a>`
						}
