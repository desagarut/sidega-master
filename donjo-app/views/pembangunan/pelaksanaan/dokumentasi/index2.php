  ...
  <script>
    $(function() {
      let tabelDokumentasi = $('#tabel-dokumentasi').DataTable({
        ...
        'ajax': {
          'url': "<?= site_url("pembangunan_dok/show/{$musdus_dok->id}") ?>",
          'method': 'POST'
        },
        'columns': [{
            'data': null
          },
          {
            'data': function(data) {
              return `<a href="<?= site_url("pembangunan_dok/form/"); ?>${data.id}" title="Edit Data"  class="btn bg-orange btn-box btn-sm"><i class="fa fa-edit"></i> </a>
								<a href="#" data-href="<?= site_url("pembangunan_dok/delete/{$musdus_dok->id}/"); ?>${data.id}" class="btn bg-maroon btn-box btn-sm"  title="Hapus" data-toggle="modal" data-target="#confirm-delete"><i class="fa fa-trash"></i></a>
							   `
            },
            'class': 'text-center'
          },
