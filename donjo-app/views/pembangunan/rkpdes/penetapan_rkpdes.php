...
<script>
	$(document).ready(function() {
		$('#example1').DataTable({
			...
			'ajax': {
				'url': "<?= site_url('pembangunan/penetapan_rkpdes') ?>",
				'method': 'POST',
				'data': function(d) {
					d.tahun = $('#tahun').val();
				}
				...
				},
				{
					'data': function(data) {
						...
						if (data.status_rkpdes == 1) {
							status_rkpdes = `<a href="<?= site_url('pembangunan/apbdes_aktiv/') ?>${data.id}" class="btn btn-success btn-sm disabled" title="APBDes Aktiv">Ditetapkan pada <br/>RKP Desa</a>`
						} else if (data.status_rkpdes == 0) {
							status_rkpdes = `<a href="<?= site_url('pembangunan/durkp_aktiv/') ?>${data.id}" class="btn  btn-primary btn-sm disabled" title="DURKP Aktiv">Ditetapkan pada<br/>DU-RKP Desa</a>`
						} else {
							status_rkpdes = `<a href="<?= site_url('pembangunan/durkp_aktiv/') ?>${data.id}" class="btn btn-warning btn-sm disabled" title="DURKP Aktiv">Silahkan Tetapkan <br/>RKP Desa / DU-RKP Desa</a>`
						}
